...
@Component({
  ...
  template: '  <!DOCTYPE html>\n' +
    ...
    '      <form class="space-y-5 mt-5" (ngSubmit)="onSubmit()">\n' +
    '        <div class="mb-4 relative">\n' +
    '          <input id="email" class="w-full rounded px-3 border border-gray-500 pt-5 pb-2 focus:outline-none input active:outline-none" type="text" [(ngModel)]="emailOrPhone" name="emailOrPhone" (input)="checkFilled($event)" required>\n' +
    ...
    '          <input id="password" class="w-full rounded px-3 pt-5 outline-none pb-2 focus:outline-none active:outline-none input active:border-blue-500" type="password" [(ngModel)]="password" name="password" (input)="checkFilled($event)" required>\n' +
    ...
    '          <a id="togglePassword" class="text-sm font-bold text-blue-700 hover:bg-blue-100 rounded-full px-2 py-1 mr-1 leading-normal cursor-pointer" (click)="togglePassword()">show</a>\n' +
    '        </div>\n' +
    '        <div class="-m-2">\n' +
    '          <a class="font-bold text-blue-700 hover:bg-blue-200 hover:underline hover:p-5 p-2 rounded-full" href="#">Forgot password?</a>\n' +
    '        </div>\n' +
    '        <button type="submit" class="w-full text-center bg-blue-700 hover:bg-blue-900 rounded-full text-white py-3 font-medium">Sign in</button>\n' +
    '      </form>\n' +
    '    </div>\n' +
